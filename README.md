...
<br>
[📖 核心自述文件 (README)](#) &nbsp; | &nbsp; [🔮 方法论深度反思与理论边界探索](开放性问题与展望.md) &nbsp; | &nbsp; [📄 渐近理论与基于泛函的分析 (PDF 下载)](数学上的一些思考.pdf)
...
本项目严格遵循 Chernozhukov et al. (2018) 的经典 DML 理论，通过 R 语言自主构建了包含 5 种数据生成过程 (DGP) 和 5 类底层机器学习算法的大规模蒙特卡洛仿真（Monte Carlo Simulation）系统，科学界定了不同机器学习模型在因果推断中的能力。
...
* **DML2：** 全局池化 (Pooling) 所有折的正交残差后进行统一回归。仿真数据证明了 DML2 在有限小样本下具有更小的方差和更优的稳定性。（这与作者Chernozhukov所述一致）
...
3. **自适应动态权重分配：** Stacking能够根据不同情形的复杂程度自动调节基学习器权重。如在线性场景中 NN 占主导，而在深层非线性场景中，NN 与 BOSTING的权重自动提升。
